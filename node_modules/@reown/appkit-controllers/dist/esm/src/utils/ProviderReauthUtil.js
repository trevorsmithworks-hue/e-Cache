 ...
 */
export function isUnauthorizedProviderError(err, depth = 0) {
 ...
 */
async function reassertActiveChain(provider) {
 ...
 */
export function withCoinbaseReauth(provider) {
    const raw = provider;
    if (!provider || typeof raw.request !== 'function') {
        return provider;
    }
    if (provider[REAUTH_WRAPPED]) {
        return provider;
    }
 ...
 */
export function maybeWrapCoinbaseProvider(params) {
    const { connectorId, chainNamespace, provider } = params;
    const shouldWrap = chainNamespace === 'eip155' &&
        isCoinbaseConnectorId(connectorId) &&
        typeof provider === 'object' &&
        provider !== null;
    return shouldWrap ? withCoinbaseReauth(provider) : provider;
}
//# sourceMappingURL=ProviderReauthUtil.js.map