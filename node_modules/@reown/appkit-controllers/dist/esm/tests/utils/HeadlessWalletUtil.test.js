...
describe('HeadlessWalletUtil.fetchWallets', () => {
    ...
    it('lists from page 1 and clears prior search when no search term', async () => {
        const fetchByPage = vi
            ...
            .mockResolvedValue(undefined);
        ApiController.state.search = [{ id: 'stale' }];
...
describe('HeadlessWalletUtil.connect', () => {
    it('connects an injected wallet via its connector', async () => {
        ...
        vi.spyOn(ConnectorController, 'getConnector').mockReturnValue(connector);
    ...
    it('deeplinks on mobile when the wallet declares a mobile link', async () => {
        ...
        expect(onConnectMobile).toHaveBeenCalledWith(expect.objectContaining({ mobile_link: 'somewallet://' }), 'https://pay/x');
    });
    it('redirects via MobileWalletUtil on mobile when there is no mobile link', async () => {
        ...
        vi.spyOn(OptionsController, 'state', 'get').mockReturnValue({ enableCoinbase: true });
        const redirect = vi
    ...
    it('clears connectingWallet and rethrows on failure', async () => {
        vi.spyOn(ConnectorController, 'getConnector').mockReturnValue({ id: 'io.metamask' });
    ...
    it('falls back to the active chain when no namespace is given', async () => {
        const getConnector = vi
            .spyOn(ConnectorController, 'getConnector')
            .mockReturnValue({ id: 'io.metamask' });
        vi.spyOn(ConnectorControllerUtil, 'connectExternal').mockResolvedValue(undefined);
        await HeadlessWalletUtil.connect(injectedWallet);
        expect(getConnector).toHaveBeenCalledWith(expect.objectContaining({ namespace: 'eip155' }));
    });
});
...
//# sourceMappingURL=HeadlessWalletUtil.test.js.map