...
import { ChainController, isCoinbaseConnectorId, isUnauthorizedProviderError, maybeWrapCoinbaseProvider, withCoinbaseReauth } from '../../exports/index.js';
// -- Helpers ------------------------------------------------------------------
const unauthorized = () => ({
    code: 4100,
...
describe('isUnauthorizedProviderError', () => {
    ...
    it('matches the eth_requestAccounts wording without a code', () => {
        expect(isUnauthorizedProviderError({
            message: "Must call 'eth_requestAccounts' before other methods"
        })).toBe(true);
    });
...
describe('withCoinbaseReauth', () => {
    ...
    it('re-asserts the active chain before retrying eth_sendTransaction', async () => {
        vi.spyOn(ChainController, 'getActiveCaipNetwork').mockReturnValue({ id: 8453 });
        let sendAttempts = 0;
        const provider = createProvider(async ({ method }) => {
...
//# sourceMappingURL=ProviderReauthUtil.test.js.map